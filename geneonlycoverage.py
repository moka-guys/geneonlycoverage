...
with open(sys.argv[1] + '.csv') as coverageall:
    ...
    line = 0
    for row in readfile:
        # create the output (will overwrite if this exists already) in new
        # folder (needs to exist in case of permissions) and write the header
        # row to it, adding sample field
        if line < 1:
            with open('geneonly/' + sys.argv[2] + '.csv', mode='w') as
            coveragegenes:
        ...
        if line > 0 and row[4] == 'Whole gene':
            with open('geneonly/' + sys.argv[2] + '.csv', mode='a') as
            coveragegenes:
                filteredfilewriter = csv.writer(coveragegenes, delimiter='\t',
                                                lineterminator='\n')
